...
import {
  ArrowUpCircle, ArrowDownCircle, Search, Receipt, Plus, X,
  BarChart3, Filter,
} from 'lucide-react';
  ...
  return (
    <div className="space-y-8 animate-fade-in">
          ...
          <div className="relative bg-white dark:bg-[#1a2332] rounded-2xl p-6 sm:p-8 w-full max-w-lg shadow-2xl animate-fade-in-up">
            ...
            <div className="flex items-center gap-3 mb-1">
              <div className={cn(
                ...
                formType === 'ENTRADA' ? "bg-emerald-100 dark:bg-emerald-900/30" : "bg-red-100 dark:bg-red-900/30"
              )}>
                {formType === 'ENTRADA'
              ...
              <h3 className="font-headline font-bold text-xl text-[#012d1d] dark:text-white">
                {formType === 'ENTRADA' ? 'Nova Entrada' : 'Nova Saída'}
              </h3>
            </div>
            <p className="text-sm text-[#414844] dark:text-white/50 mb-6 ml-[52px]">Registrar movimentação financeira</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button type="submit"
                className={cn("w-full font-bold py-3.5 rounded-xl transition-all text-sm uppercase tracking-wider flex items-center justify-center gap-2 active:scale-[0.98] shadow-lg",
                  formType === 'ENTRADA'
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => openModal('ENTRADA')}
            className="flex items-center gap-1.5 bg-emerald-500 text-white px-4 py-2.5 rounded-lg text-sm font-bold hover:bg-emerald-600 active:scale-95 transition-all shadow-md">
            <ArrowUpCircle className="w-4 h-4" /> Entrada
          </button>
          <button onClick={() => openModal('SAIDA')}
            className="flex items-center gap-1.5 bg-red-500 text-white px-4 py-2.5 rounded-lg text-sm font-bold hover:bg-red-600 active:scale-95 transition-all shadow-md">
            <ArrowDownCircle className="w-4 h-4" /> Saída
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200/50 dark:border-emerald-800/30 p-4 sm:p-5 rounded-xl hover:shadow-md hover:-translate-y-0.5 transition-all">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-8 h-8 rounded-lg bg-emerald-100 dark:bg-emerald-800/30 flex items-center justify-center">
        ...
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200/50 dark:border-red-800/30 p-4 sm:p-5 rounded-xl hover:shadow-md hover:-translate-y-0.5 transition-all">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-8 h-8 rounded-lg bg-red-100 dark:bg-red-800/30 flex items-center justify-center">
              <ArrowDownCircle className="w-4.5 h-4.5 text-red-600 dark:text-red-400" />
            </div>
        ...
        <div className={cn("p-4 sm:p-5 rounded-xl border hover:shadow-md hover:-translate-y-0.5 transition-all", balance >= 0 ? "bg-[#012d1d]/5 dark:bg-[#012d1d]/20 border-[#012d1d]/20" : "bg-red-50 dark:bg-red-900/20 border-red-200/50 dark:border-red-800/30")}>
          <div className="flex items-center gap-2 mb-2">
            <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center", balance >= 0 ? "bg-[#012d1d]/10 dark:bg-[#012d1d]/30" : "bg-red-100 dark:bg-red-800/30")}>
              <Receipt className={cn("w-4.5 h-4.5", balance >= 0 ? "text-[#012d1d] dark:text-white" : "text-red-600")} />
            </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          ...
          {(['TODOS', 'ENTRADA', 'SAIDA'] as FilterType[]).map(f => (
            <button key={f} onClick={() => setTypeFilter(f)}
              className={cn("px-3 py-2 text-[10px] font-bold uppercase tracking-wider rounded-lg transition-all",
                typeFilter === f ? "bg-[#012d1d] text-white shadow-sm" : "bg-white dark:bg-white/10 text-[#414844] dark:text-white/60 border border-[#edeeed] dark:border-white/10 hover:bg-gray-50 dark:hover:bg-white/15"
              )}>
              {f === 'SAIDA' ? 'SAÍDA' : f}
            </button>
          ))}
        ...
        {categories.length > 1 && (
          <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)}
            className="bg-white dark:bg-white/10 border border-[#edeeed] dark:border-white/10 rounded-lg px-3 py-2 text-xs font-bold text-[#414844] dark:text-white focus:outline-none transition-all">
